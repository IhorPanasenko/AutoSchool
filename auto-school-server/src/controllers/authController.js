...
const { oauth2Client, generateGoogleAuthUrl, peopleApi } = require("../config/googleOauth2Client.js");

const signSaveTokens = (res, userId, role) => {
    const accessToken = jwt.sign({ userId, role }, process.env.JWT_ACCESS_TOKEN_SECRET, {
    ...
    const accessTokenCookieExpireDate = new Date(Date.now() + process.env.JWT_ACCESS_TOKEN_COOKIE_EXPIRES * 1000);
...
exports.signup = async (req, res, next) => {
    ...
    try {
        ...
        const { accessToken, refreshToken, expire } = signSaveTokens(res, newUserAccount._id, newUserAccount.role);
        ...
        await sendVerificationEmailToken(req, newUserLogin[0], newUserAccount[0].name);
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!userLoginData || !(await userLoginData.verifyPassword(password, userLoginData.passwordHash))) {
        return next(new AppError("Incorrect email or password", 401));
    }

    const userAccountData = await userAccount.findById(userLoginData.userId);

    const { accessToken, refreshToken, expire } = signSaveTokens(res, userAccountData._id, userAccountData.role);
...
exports.getAccessToken = catchAsync(async (req, res, next) => {
    ...
    if (!refreshToken) {
        return next(new AppError("You need refresh token to gain a new access token", 401));
    }

    jwt.verify(refreshToken, process.env.JWT_REFRESH_TOKEN_SECRET, async (err, data) => {
        if (err) {
            return next(new AppError("Refresh token is no longer valid. Login to have get new tokens", 403));
        }

        const userLoginData = await userLogin.find({ userId: data.userId }).select("refreshToken");

        if (userLoginData[0].refreshToken !== refreshToken) {
            return next(new AppError("Refresh tokens don\t match or it doesn't exist", 403));
        }
        ...
        const accessToken = jwt.sign({ userId: user._id, role: user.role }, process.env.JWT_ACCESS_TOKEN_SECRET, {
            expiresIn: process.env.JWT_ACCESS_TOKEN_EXPIRES
        });
...
exports.logout = catchAsync(async (req, res, next) => {
    ...
    const userLoginData = await userLogin.findOne({ userId: req.user._id }).select("refreshToken");
...
exports.verifyEmail = catchAsync(async (req, res, next) => {
    ...
    if (!userLoginData) return next(new AppError("There is no user with that id", 400));

    if (!req.query.token) return next(new AppError("Please, provide confirmation token", 400));
    ...
    res.status(200).json({ message: "Your email address was verified successfully" });
});
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!userLoginData) return next(new AppError("No user was found with that email", 400));
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    if (!userLoginData) return next(new AppError("Token is invalid or has expired", 400));

    const userAccountData = await userAccount.findById(userLoginData.userId).select("role");

    const { accessToken, refreshToken, expire } = signSaveTokens(res, userLoginData.userId, userAccountData.role);
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!(await userLoginData.verifyPassword(req.body.currentPassword, userLoginData.passwordHash)))
        return next(new AppError("The password is wrong", 401));

    const { accessToken, refreshToken, expire } = signSaveTokens(res, req.user._id, req.user.role);
...
exports.getGoogleToken = catchAsync(async (req, res, next) => {
    ...
    const user = await userLogin.findOneAndUpdate({ email }, { googleRefreshToken: tokens.refresh_token });
